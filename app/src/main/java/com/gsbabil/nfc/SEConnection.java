...
import static com.gsbabil.nfc.Hex.toHex;
...
public class SEConnection implements CardConnection{
	...
	private Class<?> nfcExtrasClazz;
	private Object nfcAdapterExtras;
	private Object mExecEnv;
    ...
    public SEConnection(SETerminal terminal) {
		...
		NfcAdapterExtras nfcAdapterExtras = NfcAdapterExtras.get(NfcAdapter.getDefaultAdapter(mContext));
		NfcExecutionEnvironment mExecEnv = nfcAdapterExtras.getEmbeddedExecutionEnvironment();
		nfcAdapterExtras.setCardEmulationRoute(new CardEmulationRoute(CardEmulationRoute.ROUTE_ON_WHEN_SCREEN_ON, mEe));
		*/
        ...
        try {
            ...
            mExecEnv = getEEMethod.invoke(nfcAdapterExtras, (Object[]) null);
            
            // public byte[] transceive(byte[] in) throws IOException {
            Class<?> seClazz = mExecEnv.getClass();
            openMethod = seClazz.getMethod("open", (Class<?>[]) null);
            transceiveMethod = mExecEnv.getClass().getMethod("transceive",
                    new Class<?>[] { byte[].class });
            closeMethod = seClazz.getMethod("close", (Class<?>[]) null);
            
        	openMethod.invoke(mExecEnv, (Object[]) null);
            
        } catch (InvocationTargetException e) {
            Log.e(TAG, "Error: " + e.getCause().getMessage().toString(), e);
            this.terminal = null;
            return;
//            throw new RuntimeException(e);
        } catch (Exception e) {
            Log.e(TAG, "Error: " + e.getMessage().toString(), e);
            this.terminal = null;
            return;
//            throw new RuntimeException(e);
        }
    ...
    public boolean disconnect(boolean arg0) throws TerminalException {
        try {
            closeMethod.invoke(mExecEnv, (Object[]) null);
    ...
    private byte[] sendApdu(byte[] commandApdu) {
        try {
            ...
            Object response = transceiveMethod.invoke(mExecEnv, commandApdu);
